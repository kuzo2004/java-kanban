...
class InMemoryTaskManagerTest {
    ...
    public void testAddAndFindDifferentTaskTypes() {
        ...
        Task actualEpic = taskManager.getTaskById(epic.getId());
        Task actualSubtask = taskManager.getTaskById(subtask.getId());

        // проверяем поиск по id
        assertNotNull(actualTask, "Задача TASK должна быть найдена");
        assertNotNull(actualEpic, "Задача EPIC должна быть найдена");
        assertNotNull(actualSubtask, "Задача SUBTASK должна быть найдена");
    }
    ...
    public void testTasksWithManualAndAutoGeneratedIdDoNotConflict() {
        ...
        Task foundTaskWithManualId = taskManager.getTaskById(100);
        assertNotNull(foundTaskWithManualId, "Задача с заданным id должна быть найдена");

        // Проверяем, что задача с автоматически сгенерированным id доступна по своему id
        Task foundTaskWithAutoGeneratedId = taskManager.getTaskById(taskWithAutoGeneratedId.getId());
        assertNotNull(foundTaskWithAutoGeneratedId, "Задача с автоматически сгенерированным id должна быть найдена");
